        ...
        <div className={styles.squareWrap}>
          <Square
            ...
            onClick={() => onpenModal({
              ...
              functionTitle: "맡은기능",
              functionName: "- 사용자가 입력하는 데이터를 유효성 검사를 거쳐 회원가입 기능 구현 <br /> - Jotai를 활용해 회원 정보를 전역 상태에서 불러와 검증한 후, API 요청을 통해 로그인 기능 구현 <br /> - Firebase에서 제공하는 Firebase Authentication 서비스를 사용하여 휴대폰 번호로 비밀번호 찾기 / 아이디 찾기 구현 <br /> - 포인트로 구매한 상품을 ‘사용 중’과 ‘사용 완료’ 상태로 분류하여 보관할 수 있는 기능을 구현 <br /> - React-Query에서 제공하는 useInfiniteQuery 훅을 사용하여 무한스크롤 구현",
            })} />
          <Square
            ...
            onClick={() => onpenModal({
              ...
              functionTitle: "맡은기능",
              functionName: "- 로그인한 회원이 제목과 내용 작성과 Multer를 이용해 이미지를 업로드 할 수 있는 게시글 작성 기능을 구현 <br /> - 게시글의 고유한 ID값을 요청 받아 데이터베이스에서 해당 데이터를 조회하고 Sequelize를 사용하여 서버에서 수정 및 삭제 기능을 구현 <br /> - 한 게시글에 대한 댓글과 대댓글을 작성할 수 있도록 게시글과 댓글간에 1:N관계를 설정하여 댓글과 대댓글은 회원 테이블의 고유 ID와 외래키를 작성자와 닉네임을 매칭하며, 각 댓글과 대댓글은 게시글에 속할 수 있도록 글과 대댓글간에 관계를 설정하여 기능 구현 <br /> - 사용자가 입력한 제목과 게시글 내용 검색어를 서버로 쿼리 스트링을 전송하여 검색 기능 구현 <br /> - 현재 페이지의 URL경로를 확인하여 쿼리 문자열을 제외한 경로만 추출하여 경로에 따라 다른 카테고리 값을 할당하여 특정 카테고리 번호를 결정하는 로직을 구현<br /> - 한페이지에 표시할 데이터 수와 현재 페이지 값을 쿼리스트링으로 서버에 전달받아 데이터를 데이터베이스에서 조회하여 반환하여 페이지네이션 기능 구현",
            })} />
          <Square
            date="도넛 커뮤니티 사이트"
            ...
            onClick={() => onpenModal({
              ...
              functionTitle: "맡은기능",
              functionName: "- TypeScript의 정적 타입검사를 활용하여 게시판 데이터 구조를 미리 정의하여 타입 오류를 방지하고 JavaScript로 빌드하여 실제 웹 환경에서 동작하도록 구현 <br /> - 로컬스토리지에 저장된 게시판 데이터를 정의한 타입에 맞게 값을 받아와 나누어 보여주기 위해 페이지 네이션 로직 구현 <br /> - 사용자가 입력한 검색어를 쿼리 파라미터로 전달하여 해당 검색어가 포함된 제목을 필터링을 할때 타입스크립트의 타입검사를 활용 하여 검색 기능 구현",
            })} />
        </div>
